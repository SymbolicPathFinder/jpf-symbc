 ...
 *
 */
public class SymbolicSequenceListener2 extends PropertyListenerAdapter implements PublisherExtension {
	...
	public SymbolicSequenceListener2(Config conf, JPF jpf) {
		...
		for(int i=0; i<numberOfArgs; i++){
			...
			if (attribute != null){ // parameter symbolic
				...
				if(e instanceof IntegerExpression) {
					...
					System.out.println("--------->getInvokedMethod(SequenceChoiceGenerator cg) IntegerExpression:  "+solution);
				}
					...
					System.out.println("--------->getInvokedMethod(SequenceChoiceGenerator cg) else:  "+solution);
					}
				invokedMethod += solution + ",";
			}
			 // zejunzh:
			/*
			 * handle array testcases
			 */
			else if(argTypes[i].equalsIgnoreCase("int[]") || argTypes[i].equalsIgnoreCase("long[]")|| argTypes[i].equalsIgnoreCase("byte[]")
					||argTypes[i].equalsIgnoreCase("float[]") || argTypes[i].equalsIgnoreCase("double[]")||argTypes[i].equalsIgnoreCase("boolean[]")){
					 ElementInfo eiArray = (ElementInfo)argValues[i];
					 invokedMethod = invokedMethod + "new "+argTypes[i] + '{';
					 if (eiArray != null){
						 System.out.println("test null : "+eiArray.arrayLength());
                         for (int j = 0; j < eiArray.arrayLength(); j++) {
                        	 String array=eiArray.getElementAttr(j).toString();
                        	 invokedMethod=invokedMethod+array.substring(array.indexOf('[')+1, array.indexOf(']'))+',';
                         //	System.out.println("--  SymbolicSequenceListener ------: "+ eiArray.getElementAttr(j)+","+ eiArray.getFields().getFieldAttr(j)+","+eiArray.getFields().getIntValue(j));
